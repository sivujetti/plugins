...
final class JetSliders implements UserPluginInterface {
    ...
    public function __construct(UserPluginAPI $api) {
        $api->on($api::ON_ROUTE_CONTROLLER_BEFORE_EXEC, function () use ($api) {
            ...
            $api->enqueuePreviewAppJsFile("plugin-jet-sliders-webpage-preview-renderer-app-bundle.js");
        });
        $api->on($api::ON_PAGE_BEFORE_RENDER, function (Page $page, bool $editModeIsOn) use ($api) {
            if (!$editModeIsOn && !BlockTree::findBlock($page->blocks, fn($b) => $api->hasBehaviour($b, "jet-slider")))
                return;
            if (!$api->isCssFileEnqueued("sivujetti/vendor/jet-slider.bundle.css"))
                $api->enqueueCssFile("sivujetti/vendor/jet-slider.bundle.css");
            if (!$api->isJsFileEnqueued("sivujetti/vendor/keen-slider.min.js"))
                $api->enqueueJsFile("sivujetti/vendor/keen-slider.min.js");
